...
def parse_args():
    ...
    parser.add_argument( "--feature_path", default='data_buffer/feature/', type=str)
    parser.add_argument( "--label_path", default='data_buffer/label/label1.tif', type=str)
...
def main():
    ...
    print('*******************************************generate LSM*******************************************')
    model = LSM_cnn(14)
    model.load_state_dict(torch.load("hasil buffer/best.pth"))
    model.to('cuda')
